...
export function QRScanner({ open, onClose, onScanSuccess, title, description }: QRScannerProps) {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      toast.success("อ่าน QR จากรูปสำเร็จ");
    } catch (error: any) {
      console.error("File scan error:", error);
      toast.error("ไม่พบ QR ในรูป กรุณาลองใหม่ด้วยรูปที่ชัดเจน");
      setIsScanning(false);
      setMode('select');
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>{title || "สแกน QR Code"}</DialogTitle>
