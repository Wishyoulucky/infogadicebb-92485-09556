...
      // 1. Update stock (ใช้ ID ในการอัปเดต ไม่ได้ใช้ Barcode)
      if (option) {
...
      // 2. Log stock movement (ใช้ ID ในการบันทึก)
...
              </p>
                {/* เพิ่มการแสดง EAN Code/Barcode ถ้ามี (สมมติว่าคุณได้เพิ่มฟิลด์ ean_code ตามที่แนะนำ) */}
                {(option?.ean_code || product.ean_code) && (
                     <p className="text-xs text-muted-foreground">
                        EAN: {option?.ean_code || product.ean_code}
                     </p>
                )}
